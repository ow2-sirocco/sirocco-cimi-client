...
@Parameters(commandDescription = "restart machine")
public class MachineRestartCommand implements Command {
    @Parameter(description = "<machine id>", required = true)
    private List<String> machineIds;

    @Parameter(names = "-force", description = "power-off machine", required = false)
    ...
    public void execute(final CimiClient cimiClient) throws CimiClientException {
        ...
        Job job = machine.restart(this.force != null ? this.force : false);
        System.out.println("Restarting machine " + this.machineIds.get(0));
        if (job != null) {
